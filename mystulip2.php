...
<table class="table table-striped table-bordered" id="stu">
    <tbody>
        ...
        //$rows = $row_number/5 +1;
        if($row_number>0){
            for($i=0,$j=0;$i<$row_number;$i++,$j++) {

                if($i%5 == 0) echo "<tr class='.active'>";
                $row = mysqli_fetch_assoc($result);

                echo "<td><a class='a1' href=" . $row["src"] . " target=_blank>" . $row["name"] . "</a></td>";
...
<table class="table table-striped table-bordered" id="learn">
    <?php
    $sql2 = "select * from learn";
    $result2 = mysqli_query($conn,$sql2);
    $row_number = mysqli_num_rows($result2);
    //echo $row_number;
    //$rows = $row_number/5 +1;
    if($row_number>0){
        for($i=0,$j=0;$i<$row_number;$i++,$j++) {
...
<a href="manage.php" target="_blank">管理中心</a>
<!--
<p class="subtitle"><strong>汕头大学</strong></p>
<div id="stu">
